...
import 'package:poli_gestor_contenidos/screens/category_edit_screen.dart';
import 'package:poli_gestor_contenidos/screens/loading_screen.dart';
import 'package:poli_gestor_contenidos/services/auth_services.dart';
import 'package:poli_gestor_contenidos/services/categories_services.dart';
...
class _ListTags extends StatefulWidget {
  const _ListTags({
    Key? key,
    required this.tags,
  }) : super(key: key);
  ...
  @override
  State<_ListTags> createState() => _ListTagsState();
}

class _ListTagsState extends State<_ListTags> {
  @override
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          final cName = widget.tags[index].name;
          return  Padding(
            padding: const EdgeInsets.all(8),
            child: Column(
              children: [
                _TagButton(tag: widget.tags[index], onSelected: (bool value){
                  // add pick value
                  if( value ) {
                    categoryProvider.tags.add(widget.tags[index]);
                  }else{
                    categoryProvider.tags.remove(widget.tags[index]);
                  }
                  setState(() {});
...
class _TagButton extends StatefulWidget {
  ...
  const _TagButton( {required this.tag, required this.onSelected} );
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        final categoryProvider = Provider.of<CategoryProvider>(context, listen: false);
        categoryProvider.selectedTag = widget.tag.name;
      },
      child: Container(
        child: Column(
          children: [
            AnimatedContainer(
              ...
              child: Icon(
                widget.tag.icon,
                // color:  ( widget.tag.name == categoryProvider.selectedTag)
                color:  ( _isSelected == categoryProvider.selectedTag)
                ? AppTheme.secondary
                : AppTheme.primary 
                ),
            ),
